              ...
              <div className="flex flex-col overflow-y-auto" style={{ maxHeight: 520 }}>
                ...
                {entries.map((entry, idx) => (
                  <div key={entry.guestbookId} className={`flex flex-col${idx < entries.length - 1 ? ' border-b border-[#e3bfb1]' : ''}`}>
                    {/* 상단 바 — 번호 + 닉네임 + 날짜 */}
                    <div className="bg-[#f3f3f3] px-5 py-2 flex items-center gap-2 border-b border-[#e3bfb1]">
                      ...
                      />
                      {entry.isSecret && (
                        <span className="material-symbols-outlined text-[#5a4136]" style={{ fontSize: '14px' }} title="비밀글">lock</span>
                      )}
